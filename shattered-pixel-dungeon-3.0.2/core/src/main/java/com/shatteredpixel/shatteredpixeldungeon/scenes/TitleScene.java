...
import com.shatteredpixel.shatteredpixeldungeon.windows.WndRestoreGame;
...
public class TitleScene extends PixelScene {
	...
	public void create() {
		...
		// 添加 Restore 按钮
		StyledButton btnRestore = new StyledButton(GREY_TR, "Restore") {
			@Override
			protected void onClick() {
				ShatteredPixelDungeon.scene().add(new WndRestoreGame());
			}
		};
		btnRestore.icon(Icons.get(Icons.SHPX));
		add(btnRestore);
		...
		if (landscape()) {
			...
			// Restore 按钮单独一行
			btnRestore.setRect(btnAreaLeft, btnAbout.bottom() + GAP, buttonAreaWidth, BTN_HEIGHT);
			align(btnRestore);
		} else {
			...
			// Restore 按钮单独一行
			btnRestore.setRect(btnPlay.left(), btnAbout.bottom() + GAP, buttonAreaWidth, BTN_HEIGHT);
			align(btnRestore);
		}
