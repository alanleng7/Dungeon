...
public class Dungeon {
	...
	public static void deleteGame1(int save) {
		...
		// Create DeletedGame directory if it doesn't exist
		...
		if (!FileUtils.dirExists(destDir)) {
			FileUtils.getFileHandle(destDir).mkdirs();
		}

		// Move all files from source to destination
		if (FileUtils.dirExists(srcDir)) {
			// Get all files in the source directory
			ArrayList<String> files = FileUtils.filesInDir(srcDir);
...
//				}
			...
			FileUtils.deleteDir(srcDir);
		}

		// Clean up old deleted games if there are more than 5
